...
// -----------------------------------------------------------------------------
// Scintilla messages/constants used by this plugin
// -----------------------------------------------------------------------------
...
struct ViewState
{
    ...
    RECT fullRect{}; // Rectangle Notepad++ expects the Scintilla view to occupy.
    ...
    int rightNumberWidth = 0; // Mirrored native margins + separator; no extra dark gap.
...
{
    // Match the native outer border visible on the far-right side:
    // grey exterior border | left scrollbar | editor (no extra dark gap).
...
static bool CaptureMirroredMargins(ViewState& view, bool forceInvalidate = false)
{
    ...
    bitmapInfo.bmiHeader.biHeight = -height; // Top-down image.
    ...
    // Mirror the complete native margin strip in one operation. This preserves
    // every pixel and every spacing exactly: line numbers, bookmarks, fold
    // branches, change-history markers and any future native margin content.
    ...
    // A literal mirror would reverse the digits. Redraw only the line-number
    // glyphs so the layout remains perfectly mirrored while numbers stay readable.
    DrawReadableMirroredLineNumbers(view, buffer, layout, nativeWidth, height);

    // Reproduce the separator next to the native right scrollbar:
    // mirrored margins | grey border | native scrollbar (no extra dark gap).
    ...
    int childX)
{
    ...
    // The right strip is a true horizontal mirror of the native left strip.
...
static void UpdateRightNumberLayout(ViewState& view)
{
    ...
    {
        // Preserve a genuine Notepad++/plugin right-margin change, then add the
        // mirrored native margins once. This path only runs when the external
        // value differs from the value last applied by this plugin.
...
static void ApplyNormalMarginCursors(ViewState& view, bool initial)
{
    ...
    for (int margin = 0; margin < count; ++margin)
    {
        ...
        {
            // Remember a later Notepad++ preference change for restoration.
...
static void PaintRightNumbers(ViewState& view, HWND hwnd)
{
    ...
    // Copy the real Notepad++ / Scintilla bookmark margin pixel-for-pixel.
    // This preserves the exact native marker artwork, colours and theme instead
    // of drawing a separate approximation that looks bolted on.
    ...
    for (int row = 0; row < rows; ++row)
    {
        ...
        const int firstDisplayForDocument = static_cast<int>(
            SciCall(view.scintilla, SCI_VISIBLEFROMDOCLINE, documentLine));
        if (firstDisplayForDocument != displayLine)
            continue; // Wrapped continuation: native margins are blank here.
...
static void RememberAndAdjustExternalWindowPos(ViewState& view, WINDOWPOS& position)
{
    ...
    {
        // Some layout passes repeat the already-adjusted coordinates. Treat
        // those as "no logical movement" so the reserved width never grows.
...
static bool AttachView(ViewState& view, HWND scintilla)
{
    ...
    // Prevent Scintilla from painting over the mirrored child window. Without
    // this style, rapid parent repaints (notably while Ctrl is held) can flash
    // through the child and create visible flicker.
...
static void DetachView(ViewState& view)
{
    ...
    if (view.rightNumbers && ::IsWindow(view.rightNumbers))
        ::DestroyWindow(view.rightNumbers);
    if (view.leftRail && ::IsWindow(view.leftRail))
        ::DestroyWindow(view.leftRail); // Also destroys the child scrollbar.
...
static void CALLBACK SyncTimerProc(HWND, UINT, UINT_PTR, DWORD)
{
    ...
    for (auto& view : g_views)
    {
        ...
        // Styles and margin configuration change much less often than scroll
        // position. Poll them gently, then capture only after a short debounce.
...
static LRESULT CALLBACK LeftRailWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (message == WM_PAINT)
    {
        ...
        if (dc)
        {
            ...
            // Close the added left-side rail with the same grey border on
            // the exterior edge, the top edge and the bottom edge.
            if (view)
            {
                const int clientWidth = RectWidth(client);
                const int clientHeight = RectHeight(client);
    ...
    LPARAM lParam)
{
    ...
    {
        // Keep native horizontal scrolling and Ctrl+wheel zoom behaviour. The
        // guard prevents the Scintilla subclass from routing this same message
        // back into the mirrored margin handler.
    ...
    // Positive wheel delta scrolls upward, hence the negative line delta.
    SciCall(
        view.scintilla,
        SCI_LINESCROLL,
        0,
    ...
    DWORD_PTR refData)
{
    ...
    // Depending on the Windows mouse-wheel setting, the wheel message can be
    // delivered either to the hovered child or directly to focused Scintilla.
    // Handle both paths so scrolling over the mirrored margins always works.
    if (view &&
...
extern "C" __declspec(dllexport) void beNotified(SCNotificationMinimal* notification)
{
    ...
    // Any Scintilla notification may affect a native margin: text changes,
    // save-state markers, bookmarks, folding, zoom or visible-line updates.
    for (auto& view : g_views)
    {
